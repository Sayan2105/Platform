class MovieApp {
    ...
    setupEventListeners() {
        ...
        if (randomBtn) randomBtn.addEventListener('click', () => this.getRandomMovie());
        if (clearBtn) clearBtn.addEventListener('click', () => this.clearFilters());
        if (searchBtn) searchBtn.addEventListener('click', () => this.searchMovies());
    ...
    updateSliderValues() {
        ...
        if (minRuntime) {
            document.getElementById('minRuntimeValue').textContent = minRuntime.value + ' min';
        }
        if (maxRuntime) {
            document.getElementById('maxRuntimeValue').textContent = maxRuntime.value + ' min';
        }
        if (minRating) {
            document.getElementById('minRatingValue').textContent = minRating.value + ' ⭐';
        }
    }
    ...
    getRandomMovie() {
        if (this.currentMovies.length === 0) {
            this.findMovies().then(() => {
                if (this.currentMovies.length > 0) {
                    const randomMovie = this.currentMovies[Math.floor(Math.random() * this.currentMovies.length)];
